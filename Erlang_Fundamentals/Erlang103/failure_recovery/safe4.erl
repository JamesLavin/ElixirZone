...
create_backup_unless_exists(Backup, InitialState) ->
  BackupExists = is_pid(whereis(Backup)),
  if BackupExists ->
       ok;
     true ->
       BackupPid = spawn(?MODULE, backup_loop, [InitialState]),
       register(Backup, BackupPid)
  end.

create_backup_unless_exists() ->
  create_backup_unless_exists(backup, 0).
